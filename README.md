...
- **Force-quitting mid-mission doesn't get you out of it, but it does briefly go silent.** The mission's in-app sound is ordinary app audio — like any third-party app, it cannot keep playing once you swipe the app away in the app switcher; that's an iOS platform limit, not something this app (or any app) can override. What *can* survive a force-quit is AlarmKit's own native alert, since that's driven by the system rather than this app's process. While a mission is in progress the app continuously re-arms that native alert a short time out (`AlarmCoordinator.missionInsuranceDelay`, currently 15s, refreshed every 8s) — so a legitimate, still-in-progress mission never triggers a spurious extra alert, but a force-quit leaves the alarm silent for at most ~15-20s before AlarmKit's native alert (with its own sound) fires again and reopens the app. Finishing the mission (or reaching snooze/turn-off) cancels this re-arm loop.
