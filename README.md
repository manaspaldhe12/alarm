...
- The app declares the `audio` background mode (`UIBackgroundModes` in `Info.plist`), matching what other iOS alarm apps require to keep the in-mission sound playing if you merely background the app (press Home, switch apps) rather than force-quit it — without this, iOS would suspend that audio a short while after backgrounding even though the audio session itself (`AVAudioSession.Category.playback`) was already configured correctly.
...
- **Force-quitting mid-mission doesn't get you out of it.** The mission's in-app sound is ordinary app-process audio, so it can't survive the process being killed outright by a force-quit — that part is unavoidable for any app. On-device testing also found that depending on a *live* process to keep re-arming AlarmKit's native alert just-in-time isn't safe either: swiping away is instant, and a single re-arm loop can lose that race outright, especially before you've even tapped a mission button. So rather than one alarm ID being continually re-armed by a loop, the moment a mission starts (`AlarmCoordinator.startMission`) the app immediately pre-commits a whole burst of `AlarmCoordinator.insuranceShadowCount` (6) *independent* AlarmKit registrations, staggered `insuranceShadowSpacing` (12s) apart — each is its own alarm ID with its own `stopIntent`/`secondaryIntent` routing back to the real alarm. Per [Apple's own AlarmKit FAQ](https://developer.apple.com/forums/thread/797158), "all alarms are expected to persist regardless of app or device state changes, once they are successfully scheduled" — so once each individual registration call returns, a force-quit an instant later still can't wipe out more than whichever ones hadn't landed yet, unlike a single ID that depended on one more live re-arm call happening in time. This mirrors how other iOS alarm apps solve the same problem (the open-source Hjernevekker's README literally says it "will spam notifications if you quit the app" — same idea, applied to AlarmKit's own alert instead of local notifications). While the process stays alive the burst refreshes itself partway through its ~72s coverage window, and finishing the mission (or reaching snooze/turn-off) cancels every outstanding shadow so none of them fire spuriously afterward. Watch it via the **Insurance Log** (waveform icon in the alarm list toolbar), which records every registration attempt and its outcome.
- **A shadow firing while the app is closed is now actually noticed on relaunch.** The scheduling half of the burst above was only ever half the fix -- `AlarmCoordinator.syncAlertingAlarms`'s poll loop originally only recognized an alerting id if it matched a real alarm in the app's own list, and shadow ids never do (they're not "real" alarms, just insurance vehicles). That meant a shadow could be genuinely ringing at the OS level with the app completely unaware of it, unless the user happened to tap that exact alert's button -- itself not fully reliable (a documented AlarmKit issue where `stopIntent` doesn't always fire on certain dismiss gestures, developer.apple.com/forums/thread/815064). The poll loop now also checks a currently-alerting id against the persisted shadow batch and, on a match, resumes the *real* alarm's mission it was insuring -- so relaunching the app (by any means, not just tapping the alert itself) after a shadow has fired now reliably picks the mission back up.
- **A re-ring survives more than one force-quit in a row.** The re-fired alert relaunches the app back into a bare "ringing, tap a button" screen by default. `MissionInsuranceStateStore` persists which alarm has a mission genuinely in progress (written the moment you tap Snooze/Turn Off, cleared only once you actually finish it) specifically so a re-ring can check that flag and jump straight back into the same mission — restarting the insurance burst immediately — instead of waiting for another button tap first.
- **Force-quitting *before* tapping Snooze/Turn Off at all can still lose the alert once.** On-device testing found AlarmKit's own record of an actively-alerting alarm doesn't reliably survive a force-quit at that stage either — reopening the app afterward showed the alarm's AlarmKit state as completely blank ("not scheduled"), not merely silenced. Left alone, the app's own startup logic (`reconcileScheduledAlarms`) used to treat "no AlarmKit record" as "never scheduled" and silently reschedule straight for the alarm's *next* regular occurrence — so reopening the app after this happened showed nothing wrong at all, with no indication the alarm had ever been missed. It now checks whether the alarm's regular time passed within the last `AlarmCoordinator.missedAlarmGracePeriod` (20 minutes) before doing that, and if so shows the ringing screen (with mission-gating applying normally, including the insurance burst above) instead of silently rescheduling past it.
